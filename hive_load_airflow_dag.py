...
create_hive_database = DataprocSubmitHiveJobOperator(
    ...
    cluster_name='xxxxcluster',
    region='us-central1',
    project_id='dataengineering-xxxxxx',
    dag=dag,
...
create_hive_table = DataprocSubmitHiveJobOperator(
    ...
    query="""
        ...
        STORED AS TEXTFILE
        LOCATION 'gs://logistics-raw-rinkesh/input_data'
...
archive_processed_file = BashOperator(
    ...
    bash_command=f"gsutil -m mv gs://logistics-raw-rinkesh/input_data/logistics_*.csv gs://logistics-archive-rinkesh/",
    dag=dag
)
...
sense_logistics_file >> create_hive_database >> create_hive_table >> create_partitioned_table >> set_hive_properties_and_load_partitioned >> archive_processed_file >> dag_run
